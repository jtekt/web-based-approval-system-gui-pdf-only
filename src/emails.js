...
&body=${encodeURIComponent(
    generate_email_to_recipient_body(application, recipient)
  )}`;
...
申請の承認が${
    application_is_rejected(application) ? "却下" : "完了"
  }されました。
...
?subject=[電子捺印システム] ${
    application_is_rejected(application) ? "[却下]" : "[承認]"
  } ${application.title}
&body=${encodeURIComponent(generate_email_to_applicant_body(application))}`;

export { generate_email_to_applicant, generate_email_to_recipient };
