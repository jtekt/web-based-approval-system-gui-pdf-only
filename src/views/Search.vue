      ...
      <v-card outlined>
        ...
        <v-form @submit.prevent="search()">
          <v-card-text>
            ...
            <v-row align="center">
              <v-col cols="auto">{{ $t("Applicant group") }}</v-col>
              <v-col cols="auto">
                <v-chip
                  v-if="selected_group"
                  close
...
export default {
  ...
  methods: {
        ...
        .then(({ data }) => {
          ...
          this.applications.forEach((application) => {
            ...
            form_data.forEach((field) => {
              if (
                !this.field_labels.includes(field.label) &&
                field.type !== "file"
              ) {
    ...
    row_clicked({ _id }) {
      this.$router.push({
        name: "application",
        params: { application_id: _id },
      })
    },
    export_table() {
      var workbook = XLSX.utils.book_new()
      var ws1 = XLSX.utils.table_to_sheet(
        document.getElementById("search_results_table")
      )
